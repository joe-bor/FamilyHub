...
export function useCalendarEvents(
  ...
  options?: Omit<UseQueryOptions<ApiResponse<CalendarEvent[]>, ApiException>, "queryKey" | "queryFn">
) {
  return useQuery({
...
export function useUpdateEvent(callbacks?: UpdateEventCallbacks) {
  ...
  return useMutation({
    ...
    onMutate: async (updatedEvent) => {
      ...
      const previousData = queryClient.getQueriesData<ApiResponse<CalendarEvent[]>>({
        queryKey: calendarKeys.events(),
      })

      queryClient.setQueriesData<ApiResponse<CalendarEvent[]>>({ queryKey: calendarKeys.events() }, (old) => {
        if (!old) return old
        return {
          ...old,
          data: old.data.map((event) =>
            event.id === updatedEvent.id
              ? {
                  ...event,
                  ...updatedEvent,
                  date: updatedEvent.date ? new Date(updatedEvent.date) : event.date,
                }
              : event
          ),
        }
